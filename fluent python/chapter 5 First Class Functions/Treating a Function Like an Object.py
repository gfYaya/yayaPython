...
def factorial(n):
    '''returns n1'''
    '''ssss'''  # It's not printed in the console as the interpreter run the 'print(factorial.__doc__)' operation
...
print(map_a)
print(list(map_a))  # factorial returns 1 if n == 1 else n * factorial(n - 1) ,it gets bitten by the
# bug ErrorRecursion,because the first value is zero,then the function pass a negative argu in
# the next step with Tail Recursion
